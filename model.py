...
def decode_ids(ids, itos):
    ...
    st = []
    for my_id in ids:
        st.append(decode_int(my_id ,itos ))
    result = "".join(st)
    return result
...
def naive_softmax_1d(logits):
    ...
    vals = array_exp(logits)
    return array_exp(logits) /sum_all(vals)
...
def softmax_overflow_demo(large_value):
    ...
    ov_fl = np.exp(large_value)
    if ov_fl == float('inf'):
        overFlow = True
    else:
        overFlow= False
    return {'naive_exp': ov_fl , 'overflowed': overFlow}
...
def stable_softmax_1d(logits):
    ...
    arr_exp = array_exp((logits - max_val))
...
def stable_softmax_2d_rowwise(logits):
    ...
    arr_exp = array_exp((logits-max_val)) 
    return arr_exp / sum_keepdims( arr_exp, 1)
...
def read_text_file(text_blob):
    ...
    if not text_blob :
        ...
        raise TypeError(f"input must be string , got{type(text_blob).__name__}")
    return text_blob
...
def encode_corpus_to_int_array(text, stoi):
    ...
    listi = []
    for st in text : 
        listi.append(stoi[st])
    return np.array(listi , dtype=np.int64)
...
def sample_random_batch_offsets(data_len, block_size, batch_size, rng):
    ...
    idx_start = data_len-block_size
    idx = rng.integers(0 ,idx_start+1 , size = batch_size )
    return idx
...
def stack_x_batch(data, offsets, block_size):
    ...
    for i in range(len(offsets)):
        listi.append(slice_x_at_offset(data , offsets[i],block_size))
...
def stack_y_batch(data, offsets, block_size):
    ...
    listi = []
    for i in range(len(offsets)):
        listi.append(slice_y_at_offset(data , offsets[i] , block_size))
    return np.array(listi)
